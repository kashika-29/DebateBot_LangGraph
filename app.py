...
# Initialize API keys and model configuration
GROQ_API_KEY = os.getenv("GROQ_API_KEY", "").strip().rstrip(";")
TAVILY_API_KEY = os.getenv("TAVILY_API_KEY", "").strip().rstrip(";")
...
6. Cite facts and evidence directly in your text
...
2. Note that final persuasion scores determine the winner (higher score wins; equal scores tie).
3. For "Best Argument", select ONE exact quote from the Pro Agent's messages that was particularly strong
4. For "Best Rebuttal", select ONE exact quote from the Con Agent's messages that was particularly effective
5. For "Pro Strengths", list 3-4 specific strengths based on actual arguments made
6. For "Con Strengths", list 3-4 specific strengths based on actual arguments made
7. Do NOT use generic phrases like "Not specified" - always extract from the actual debate

Provide your verdict in EXACTLY this format:

Winner:
[Pro Agent or Con Agent or Tie]
...
def calculate_authoritative_winner(pro_score: float, con_score: float) -> str:
    """Calculate the authoritative winner based strictly on numerical scores"""
...
"""
        ...
        # DEBUG: Print raw Judge response safely for Windows console
        try:
            print("=" * 50)
            print("DEBUG: Raw Judge Response:")
            print("=" * 50)
            print(verdict_text.encode(sys.stdout.encoding or 'utf-8', errors='replace').decode(sys.stdout.encoding or 'utf-8'))
            print("=" * 50)
        except Exception:
            pass
        ...
        # Parse Winner from LLM text if available
        ...
        # ------------------------------------------------------------------
        # AUTHORITATIVE WINNER OVERRIDE RULE
        # The final numerical scores strictly determine the winner in Python.
        # Any inconsistent LLM-generated Winner string is overridden.
        # ------------------------------------------------------------------
        pro_score_val = int(round(st.session_state.get('pro_score', 50.0)))
        con_score_val = int(round(st.session_state.get('con_score', 50.0)))
        
        # Check if LLM output explicitly specifies numerical scores
        score_match = re.search(r'Pro:?\s*(\d+).*?Con:?\s*(\d+)', verdict_text, re.IGNORECASE)
        if score_match:
            try:
                pro_score_val = int(score_match.group(1))
                con_score_val = int(score_match.group(2))
            except Exception:
                pass
        
        authoritative_winner = calculate_authoritative_winner(pro_score_val, con_score_val)
        verdict['winner'] = authoritative_winner
        verdict['final_score'] = f"Pro: {pro_score_val} - Con: {con_score_val}"
        ...
        return {
            ...
            'final_score': f"Pro: {pro_score_val} - Con: {con_score_val}",
...
def search_tavily(query: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if isinstance(results, str):
            if "HTTPError" in results or "401" in results or "Unauthorized" in results:
                return []
            return [{'source': 'Tavily Search', 'url': '', 'content': results}]
        ...
        if isinstance(results, list):
            for result in results:
                if isinstance(result, dict):
                    citations.append({
                        'source': result.get('title') or result.get('source', 'Unknown'),
                        'url': result.get('url', ''),
    ...
    if st.session_state.debate_active:
        ...
        with col2:
            if st.session_state.current_turn == 'pro' and not st.session_state.pro_spoken_this_round:
                if st.button("▶️ Next Turn (Pro)", key="next_turn_pro", use_container_width=True, type="primary"):
                    simulate_pro_turn()
                    st.rerun()
            elif st.session_state.current_turn == 'con' and not st.session_state.con_spoken_this_round:
                if st.button("▶️ Next Turn (Con)", key="next_turn_con", use_container_width=True, type="primary"):
                    simulate_con_turn()
                    st.rerun()
...
def render_verdict_page():
    ...
    verdict = st.session_state.judge_verdict or {}
